#!/bin/bash
MODULE_NAME=shutit_vpn_example
...
then
	while true
	do
		VBoxManage list runningvms | grep ${MODULE_NAME} | awk '{print $1}' | xargs $XARGS_FLAG -IXXX VBoxManage controlvm 'XXX' poweroff && VBoxManage list vms | grep shutit_vpn_example | awk '{print $1}'  | xargs -IXXX VBoxManage unregistervm 'XXX' --delete
